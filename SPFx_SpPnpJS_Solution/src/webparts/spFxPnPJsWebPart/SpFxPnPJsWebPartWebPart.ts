...
import * as strings from 'SpFxPnPJsWebPartWebPartStrings';
import SpFxPnPJsWebPartViewModel, { ISpFxPnPJsWebPartBindingContext } from './SpFxPnPJsWebPartViewModel';
import pnp from "sp-pnp-js";

let _instance: number = 0;

export interface ISpFxPnPJsWebPartWebPartProps {
  description: string;
}

export default class SpFxPnPJsWebPartWebPart extends BaseClientSideWebPart<ISpFxPnPJsWebPartWebPartProps> {
  private _id: number;
  ...
  protected onInit(): Promise<void> {
    ...
    const bindings: ISpFxPnPJsWebPartBindingContext = {
  ...
  private _registerComponent(tagName: string): void {
    ko.components.register(
      tagName,
      {
        viewModel: SpFxPnPJsWebPartViewModel,
        template: require('./SpFxPnPJsWebPart.template.html'),
        synchronous: false
      }
    );
  }
